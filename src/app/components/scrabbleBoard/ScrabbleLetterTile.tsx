...
export default function ScrabbleLetterTile(
    ...
    return (
        <div
            ref={tileRef}
            onMouseDown={moveLetter ? (e: React.MouseEvent) => moveLetter(e) : undefined}
            onTouchStart={moveLetter ? (e: React.TouchEvent) => moveLetter(e) : undefined}
            className={
                `w-full h-full flex justify-center items-center shadow-orange-50/75
                text-center border text-white overflow-hidden ${background}`
            }
        >
            {blank ? "(" + letter + ")" : letter}
        </div>
